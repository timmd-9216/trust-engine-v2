#!/usr/bin/env bash
# Update the process-posts-hourly Cloud Scheduler job URI (e.g. to change max_posts_to_process).
# Loads PROJECT_ID, REGION and optional SERVICE_NAME from .env; gets SERVICE_URL via gcloud.
#
# Usage:
#   ./scripts/update_scheduler_process_posts_uri.sh [max_posts_to_process]
#   Default max_posts_to_process=20 if not given.
#
...
NEW_URI="${SERVICE_URL}/process-posts?max_posts_to_process=${MAX_POSTS_TO_PROCESS}"
echo "Updating process-posts-hourly to: ${NEW_URI}"
